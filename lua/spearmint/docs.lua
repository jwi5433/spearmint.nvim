...
local Groups = require("spearmint.groups")
...
function M.extras()
  local Extra = require("spearmint.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/spearmint/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/spearmint/config.lua", "\n(--@class spearmint%.Config.-\n})")
